...
class Match(Base):
    ...
    _winning_seed = relationship('Seed', primaryjoin="and_(Match.WTeamID==Seed.TeamID, Match.Season==Seed.Season)")
    winning_seed = association_proxy('_winning_seed', 'Seed')
    losing_team = relationship('Team', foreign_keys=[LTeamID])
    _losing_seed = relationship('Seed', primaryjoin="and_(Match.LTeamID==Seed.TeamID, Match.Season==Seed.Season)")
    ...
    def _return_stats(self, cats):
        stats = {}
        for cat in cats:
            stats[stat_dict[cat]] = self.__getattribute__(cat)
        return stats

    def winning_stats(self):
        cats = ['id', 'Season', 'mdid', 'stage', 'WTeamID', 'WScore', 'NumOT',
                'WFGP', 'WFGP_avg', 'WFGP3', 'WFGP3_avg',
                'WR', 'WR_avg']
